...
| **TTS & Audio** | `text-to-speech`, `speech-to-text` (whisper, default STT), `azure-speech-to-text` (optional cloud STT), `music`, `sound-effects`, `elevenlabs`, `agents`, `setup-api-key`, `mlx-audio` (local Apple Silicon TTS) | `elevenlabs/skills`, `digitalsamba/claude-code-video-toolkit`, Local OpenMontage skill |
