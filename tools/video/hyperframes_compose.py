...
class HyperFramesCompose(BaseTool):
    ...
    def _scaffold(self, inputs: dict[str, Any]) -> ToolResult:
        """Materialize the HyperFrames workspace from Balamonis Studio artifacts.
        ...
        gsap_src = self._stage_gsap(workspace)
        html = self._generate_index_html(
            ...
            or f"Balamonis Studio {edit_decisions.get('renderer_family', 'composition')}",
        )
    ...
    def _stage_gsap(self, workspace: Path) -> str:
        """Stage GSAP into the workspace; return how index.html should load it.

        The generated timeline depends on GSAP. Loading it from a CDN
        (jsdelivr) is fragile: it breaks behind a restricted-egress proxy and
        is fail-closed in distributed/cloud renders. We stage a vendored,
        pinned copy and reference it relatively so the composition renders
        offline and deterministically, falling back to the CDN only if the
        vendored copy is missing.
        """
        name = stage_gsap(workspace / "vendor")
        return f"vendor/{name}" if name else GSAP_CDN_FALLBACK

    @staticmethod
    def _font_stack(font: str) -> str:
        """Build a CSS font-family value with resolvable fallbacks.

        HyperFrames' static guard resolves literal family names — not CSS
        variables — and requires each family to be bundled/auto-resolved or
        backed by an @font-face. Emitting bare `var(--font-heading)` therefore
        trips a `font_family_without_font_face` contract error. We emit the
        chosen family followed by `Inter` (a bundled family) and the generic
        `system-ui, sans-serif`, so text always resolves and renders.
        """
        font = (font or "").strip() or "Inter"
        parts = [font]
        if font.lower() != "inter":
            parts.append("Inter")
        parts += ["system-ui", "sans-serif"]
        return ", ".join(
    ...
    def _style_bridge(
        ...
        """Bridge Balamonis Studio playbook → HyperFrames CSS vars + DESIGN.md.
        ...
        design_md = (
            "# DESIGN\n\n"
            "Generated by Balamonis Studio HyperFrames style bridge (fallback).\n\n"
            f"- Background: `{bg}`\n"
            f"- Foreground: `{fg}`\n"
            f"- Accent: `{accent}`\n"
            f"- Primary: `{primary}`\n"
    ...
    def _generate_index_html(
        ...
        # Emit literal, resolvable font stacks. The static guard cannot see
        # through CSS variables, so `font-family: var(--font-heading)` reads as
        # an undeclared family and fails validation.
        heading_stack = self._font_stack(css_vars.get("--font-heading", "Inter"))
        body_stack = self._font_stack(css_vars.get("--font-body", "Inter"))
...
<head>
  ...
  <style>
    ...
    body {{ margin: 0; background: var(--color-bg); color: var(--color-fg); font-family: {body_stack}; }}
    ...
    .clip.text-card h1 {{ font-family: {heading_stack}; font-weight: 700; font-size: 96px; line-height: 1.1; margin: 0; color: var(--color-fg); }}
...
"""
    ...
    @staticmethod
    def _resolve_local_chromium() -> Optional[str]:
        """Find a pre-installed (Playwright-managed) Chromium, if any.

        Returns the path to a headless-shell or full Chromium binary under
        PLAYWRIGHT_BROWSERS_PATH (or the conventional /opt/pw-browsers
        location), or None if none is present. Used to avoid HyperFrames
        having to download its own Chrome in restricted-egress/offline
        environments.
        """
        roots = [os.environ.get("PLAYWRIGHT_BROWSERS_PATH"), "/opt/pw-browsers"]
        # Prefer the headless shell (what HyperFrames downloads itself), then
        # fall back to a full Chromium build.
        for prefix, binary in (("chromium_headless_shell", "headless_shell"), ("chromium", "chrome")):
            for root in roots:
                ...
                for entry in entries:
                    if not entry.startswith(prefix):
                        continue
                    candidate = os.path.join(root, entry, "chrome-linux", binary)
    ...
    def _subprocess_env(cls) -> dict[str, str]:
        """Environment for `npx hyperframes`, hardened for sandboxed/CI hosts.

        Two defaults make `npx hyperframes` work where it otherwise can't,
        without changing behaviour on a normal developer machine:

        * ``ONNXRUNTIME_NODE_INSTALL=skip`` — the `hyperframes` package depends
          on `onnxruntime-node`, whose postinstall downloads a large native
          binary from a CDN. When egress is restricted that download fails and
          aborts the *entire* `npx hyperframes` install, even though onnx only
          powers optional *local* TTS/music fallbacks (Kokoro, MusicGen) the
          core HTML/CSS/GSAP render never touches. We skip it by default; a
          user who wants those local-AI fallbacks can export their own value.
        * ``HYPERFRAMES_BROWSER_PATH`` — point HyperFrames at a pre-installed
          Chromium when one is present, so it doesn't try to download Chrome.
          Only set when the user hasn't and a browser is actually found.
        """
        env = dict(os.environ)
        env.setdefault("ONNXRUNTIME_NODE_INSTALL", "skip")
        if not env.get("HYPERFRAMES_BROWSER_PATH"):
            browser = cls._resolve_local_chromium()
            if browser:
