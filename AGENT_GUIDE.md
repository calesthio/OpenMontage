...
| `tools.audio.mlx_audio_tts` | `MLXAudioTTS` | ~~MLXAudioTTSTool~~ |
...
|----------|-----------|-----------------|
| `tts_selector` | All tools with `capability="tts"` (ElevenLabs, Google TTS, OpenAI, Piper, MLX-Audio) | `registry.get_by_capability("tts")` |
...
| **Audio** | `elevenlabs`, `music`, `sound-effects`, `acestep`, `text-to-speech`, `setup-api-key`, `mlx-audio` (local Apple Silicon TTS) |
