...
- **Silent runtime swap is forbidden.** If `render_runtime="hyperframes"` was locked and HyperFrames is unavailable, do NOT route to Remotion instead. `video_compose` re-checks availability at compose time and returns a **structured blocker** (`data["blocker"] == "runtime_unavailable"`, with `locked_runtime`, `available_runtimes`, `missing_requirements`, and the three options: install / downgrade / abort). Surface those options, get **explicit** user approval, then — and only then — record the downgrade in `edit_decisions.runtime_availability_override` **and** append a revised `decision_log` entry with `category: "render_runtime_selection"` and the **same subject** as the original runtime decision (Re-log Changed Decisions — do not invent a new category), carrying the unavailable runtime in `options_considered`/`rejected_because` and the approved downgrade as `selected` with `user_approved: true`. Pass that `decision_log` into `video_compose`; compose validates the evidence **before** dispatch and proceeds only to the `accepted_runtime` the user named.
...
When `render_runtime="hyperframes"` is locked and HyperFrames is unavailable (Node < 22, missing `ffmpeg`/`npx`, or `hyperframes doctor` reports issues), that's also a blocker. `video_compose` returns the structured `runtime_unavailable` blocker rather than swapping engines. Do not substitute Remotion or FFmpeg without explicit user approval + an `edit_decisions.runtime_availability_override` block + a revised `decision_log` entry with `category: "render_runtime_selection"` and the same subject as the original runtime decision, appended **before** re-calling compose.
