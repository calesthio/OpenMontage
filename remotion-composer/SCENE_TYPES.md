...
| **`screencast_scene`** | **`ScreencastScene`** | **`source`** (recorded capture mp4), **`screencastOverlays`** (list of `{atSeconds, untilSeconds, ...ScreenshotStep}`) | **`source_in_seconds`, `screencastSize` (natural px w/h), `screencastCursor` (`[{atSeconds,to}]`), `screencastZoom` (`[{atSeconds,untilSeconds,region,scale}]`), `cursorStartAt`, `accentColor`** | **Approach-2 — animated overlays over a MOVING recording. Reuses the `screenshot_scene` overlay primitives (highlight_box, callout_balloon, cursor, click_pulse, …) but drives them by ABSOLUTE video timestamps, plus optional zoom-to-highlight. Coordinates are normalized (0–1) against the video's contain-fit rect. Built for Cypress-driven tutorial videos: step times + real element bboxes come from the capture manifest. See [`.agents/skills/cypress-recording/SKILL.md`](../.agents/skills/cypress-recording/SKILL.md).** |
