...
- **Silent runtime swap is forbidden.** If `render_runtime="hyperframes"` was locked and HyperFrames is unavailable, do NOT route to Remotion instead. `video_compose` re-checks availability at compose time and returns a **structured blocker** (`data["blocker"] == "runtime_unavailable"`, with `locked_runtime`, `available_runtimes`, `missing_requirements`, and the three options: install / downgrade / abort). Surface those options, get **explicit** user approval, then — and only then — record the downgrade in `edit_decisions.runtime_availability_override` **and** a `decision_log` entry with `category: "runtime_availability_override"` (capturing what was locked vs. what actually ran). Compose proceeds only to the `accepted_runtime` the user named. This compose-time override is distinct from the proposal-time `render_runtime_selection` decision.
