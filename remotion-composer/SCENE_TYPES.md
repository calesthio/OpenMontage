...
| **`audiogram`** | **`AudiogramScene`** | **`audioSrc`** (path/URL of the track to visualize) | **`audiogramStyle` (bars\|line\|circle), `accentColor` (bar color), `bars`, `gain`, `numberOfSamples` (pow2), `audioOffsetSeconds`, `backgroundVideo`/`backgroundImage`** | **Music-reactive visualization powered by `@remotion/media-utils` (`visualizeAudio`). Bars/line/ring pulse to the actual audio. `audioSrc` should point to the same file as your `audio.music.src`; set `audioOffsetSeconds` to match `audio.music.offsetSeconds` so the bars line up with what's audible. Overlay it on a reel via `backgroundVideo`. `numberOfSamples` must be a power of two (default 256).** |
| **`draw_path`** | **`DrawPathScene`** | **`paths`** (list of `{ d, color?, strokeWidth?, fill? }`) | **`viewBox` (default `"0 0 1920 1080"`), `accentColor` (default stroke), `strokeWidth`, `drawDurationInFrames`, `staggerFrames`, `fillFadeInFrames`, `backgroundVideo`/`backgroundImage`** | **Self-drawing SVG powered by `@remotion/paths` (`evolvePath`). Each path's stroke animates from 0→100% like a hand sketching it; multiple paths draw in sequence via `staggerFrames`; an optional per-path `fill` fades in once its stroke completes. Powers the Ink Theater "sketch comes to life" look, animated logos, and hand-drawn line reveals. `d` values must be authored in the `viewBox` coordinate space.** |
